      ...
      try {
        ...
        if (message && !silent && parsed.data.success) {
          Toast.show({
            ...
            title:'Success',
            textBody:parsed.data.message ?? 'Operation completed successfully',
          });
        }
        await mutate(endpoint); // Revalidate cache
        return {
          data: parsed.data.data as R,
          response: parsed.data as ApiResponse<R>,
        };
      } catch (error: unknown) {
        if (
  ...
  const batch = async <R>(
    ...
    try {
      const results = await Promise.all(
        requests.map(({ method, endpoint, data }) =>
          requestInstance
            ...
            .then(res => {
              if (dataSchema) {
                const parsed = ApiResponseSchema(dataSchema).safeParse(
                  res.data,
                );
                if (!parsed.success) {
                  throw new Error(
      ...
      Toast.show({
        type:ALERT_TYPE.DANGER,
        title:'Error',
        textBody:errMsg.message || 'Batch request failed',
      });
      throw error;
    } finally {
      setIsMutating(false);
